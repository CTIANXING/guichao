...
## Day 6 — 2026-06-10：立方体交互编辑 ✅ 完成

### 计划任务

| 步骤 | 内容 | 状态 |
|------|------|------|
| 6.1 | CubeDemo 增加 Raycaster 点击检测 — 区分拖拽/点击，拾取立方体，边框高亮变色 | ✅ |
| 6.2 | 新建 UnitEditPanel — 底部面板：名称输入、8色预设、尺寸±按钮(0.2~8.0) | ✅ |
| 6.3 | 编辑面板接 storageStore — 改名/改色/改尺寸写入DB，场景原地更新 | ✅ |
| 6.4 | "摞在…上面"堆叠逻辑（下拉选父单元 + 自动对齐 posY） | ✅ |
| 6.5 | 删除按钮 | ✅ |
| 6.6 | 新立方体自动网格排布（不再全部堆在原点） | ✅ |

### 踩坑记录

| 问题 | 根因 | 解决 |
|------|------|------|
| 改色/改尺寸不生效 | `onContextCreate` 闭包捕获旧 `units`，每次 `setSceneVersion` 触发 GLView 重挂但旧动画循环未取消 | GLView `key` 改为仅按 `roomId`（房间切换才重挂），增删改由 `useEffect([units])` 原地重建立方体 |
| 新立方体全部堆在原点看不见 | 默认 pos (0,0,0) 全部重叠 | `handleAddUnit` 按 `col*2.5-3.75, row*2.5` 网格自动排布 |
| 点击穿透/改色错位 | 重叠立方体的 Raycaster 命中不可预测 | 自动排布解决重叠问题 |
| 旋转过快 | `gestureState.dx/dy` 是累计位移非帧间增量 | 改为 `deltaX = g.dx - prevDx` 帧间增量 + 灵敏度 0.005 |

### 已知限制（P1 将解决）

- 堆叠逻辑为下拉选择，不防循环引用（A摞B + B摞A 会浮空）。P1 将用**拖拽移动 + 吸附**完全替代
...
- `components/UnitEditPanel.tsx` — 新建：8色预设+尺寸调节+堆叠选择+删除
- `components/CubeDemo.tsx` — 重写：Raycaster拾取、ref桥接避免闭包、原地更新
- `app/(tabs)/room3d.tsx` — 重写：移除 sceneVersion、自动排布新立方体
- `stores/storageStore.ts` — 修改：清理调试日志
- Git commit: `1c7c29f` — "Day 6: 立方体点击选中+编辑面板+堆叠+删除+自动排布"
